...
if (file_exists($demoFile) && is_dir($demoPreviewDir)) {
    ...
    // Check if hide_banner=true is set in the .demo file
    $hideBanner = false;
    $demoContents = @file_get_contents($demoFile);
    if ($demoContents !== false && preg_match('/^\s*hide_banner\s*=\s*true\s*$/mi', $demoContents)) {
        $hideBanner = true;
    }

    // Inject a small banner linking to the Studio (unless hidden for screenshots)
    $demoBanner = '';
    if (!$hideBanner) {
